...
# --- CONTEXT VAR ---
# This safely holds the token per-request, even under heavy concurrent load
snow_token_var = contextvars.ContextVar("snow_token", default="")

# --- CONFIGURATION ---
SERVICENOW_BASE_URL = "https://dev408306.service-now.com/api/now"
SERVICENOW_USERNAME = "admin"
# We can keep this as a fallback, or remove it completely if you strictly want to use the header
...
class APIKeyMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # NEW: Extract the ServiceNow token and store it in the ContextVar
        ...
        try:
            ...
            # Always reset the context var to prevent memory leaks
            snow_token_var.reset(token_id)


# --- HELPERS ---
def _basic_auth_header() -> str:
    # Retrieve the token dynamically from the current request context
    snow_password = snow_token_var.get()
    print("password",snow_password)

    # If the header wasn't provided, use the fallback
    if not snow_password:
        snow_password = SERVICENOW_PASSWORD_FALLBACK

    token = base64.b64encode(f"{SERVICENOW_USERNAME}:{SERVICENOW_PASSWORD_FALLBACK}".encode()).decode()
...
async def create_incident(
    ...
    # (Docstring omitted for brevity - keep your existing docstring here)
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(
            ...
            json=payload,
            headers={
                "Authorization": _basic_auth_header(), # Now uses the dynamic header
