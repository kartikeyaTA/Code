...
from azure.ai.projects.models import PromptAgentDefinition, MCPTool, RaiConfig  # 🎯 IMPORT FOR CREATION BLUEPRINT + MCP TOOL


# ============================================================================
# 1. PARAMETERS & CONFIGURATION
# ============================================================================
project_endpoint = "https://voice-agent-txrh.services.ai.azure.com/api/projects/voice-agent-txrh"
...
mcp_server_url = 'https://txrh-apim-aisharedservicesgeneralprod-6279-stosup.azure-api.net/snow/'
mcp_project_connection_id = os.environ.get("MCP_PROJECT_CONNECTION_ID", "txrh-agent-tool-snow-mcp")
mcp_require_approval = os.environ.get("MCP_REQUIRE_APPROVAL", "never") # "never" | "always"
sub="c34b42f0-a89c-4e9d-9205-57ae6b91357f"
rg="VisualStudioOnline-C88866B319C044FFB02C78F550AEF0FB"
account="voice-agent-txrh"

# Construct the full ARM resource ID for Microsoft.DefaultV2 on your account
rai_policy_id = (
    f"/subscriptions/{sub}"
    f"/resourceGroups/{rg}"
    f"/providers/Microsoft.CognitiveServices/accounts/{account}"
    f"/raiPolicies/Microsoft.DefaultV2"
)
...
with AIProjectClient(
    ...
    try:
        ...
        if isinstance(current_definition, dict):
            ...
            current_definition["rai_config"] = {"rai_policy_name": rai_policy_id}
        else:
            current_definition.instructions = new_instructions
            current_definition.model = model_deployment
            current_definition.tools = ensure_mcp_tool(getattr(current_definition, "tools", None))
            current_definition.rai_config = RaiConfig(rai_policy_name=rai_policy_id)
