...
# Force stdout to flush immediately for real-time Azure DevOps log streaming
...
def handle_consent_if_needed(oai_client, response, original_prompt: str, timeout_seconds: int = CONSENT_TIMEOUT_SECONDS):
    ...
    print("\n\n❌ Timed out waiting for OAuth consent in pipeline.", flush=True)
    sys.exit(1)
...
with AIProjectClient(
    ...
    print(f"📤 Uploading local dataset '{LOCAL_DATASET_PATH}'...", flush=True)
    try:
    ...
    while True:
        ...
        if current_status in terminal_states or any(k in current_status for k in ["complete", "error", "fail"]):
            print(f" [{current_status.upper()}]", flush=True)
            break
        time.sleep(6)
    ...
    print("-" * 80, flush=True)
    print(f"   • Relevance Score:    {avg_rel:.2f} / 5.0", flush=True)
