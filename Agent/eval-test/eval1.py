...
# Azure AI Foundry Project Config (for loading draft version instructions)
PROJECT_ENDPOINT = os.environ.get("AZURE_AI_PROJECT_ENDPOINT", "https://txrh-foundry.services.ai.azure.com/api/projects/txrh-project")
AGENT_NAME = os.environ.get("AGENT_NAME", "txrh-demoagent-2")

# Read draft version ID generated by check.py
AGENT_VERSION = "draft-1786982754865"
if not AGENT_VERSION and os.path.exists("version.txt"):
...
<body>
    ...
    <div class="subtitle">Evaluated via APIM Gateway | Agent Version: <b>{AGENT_VERSION}</b> | Model: {MODEL_DEPLOYMENT}</div>
...
def main():
    ...
    print(f"📌 Evaluating Agent Target: '{AGENT_NAME}' | Draft Version: '{AGENT_VERSION}'")

    if not os.path.exists(EVAL_DATASET_PATH):
        print(f"❌ Required evaluation dataset missing: '{EVAL_DATASET_PATH}'")
        sys.exit(1)

    # 1. Load instructions from Azure AI Foundry for the specific draft version
    candidate_instructions = ""
    try:
        print(f"📖 Fetching agent definition for draft version '{AGENT_VERSION}' from Foundry...")
        with AIProjectClient(endpoint=PROJECT_ENDPOINT, credential=DefaultAzureCredential()) as proj_client:
            draft_version_obj = proj_client.agents.get_version(agent_name=AGENT_NAME, agent_version=AGENT_VERSION)
            defl = getattr(draft_version_obj, "definition", None)
            if defl:
                candidate_instructions = defl.get("instructions") if isinstance(defl, dict) else getattr(defl, "instructions", "")
                print(f"✅ Successfully retrieved draft instructions from Azure AI Foundry!")
    except Exception as err:
        print(f"⚠️ Could not fetch draft version '{AGENT_VERSION}' from Foundry ({err}). Falling back to '{PROMPT_FILE_PATH}'...")
        if os.path.exists(PROMPT_FILE_PATH):
            with open(PROMPT_FILE_PATH, "r", encoding="utf-8") as f:
                candidate_instructions = f.read().strip()
        else:
            print(f"❌ Could not load instructions.")
            sys.exit(1)

    # 2. Load tools schema for OpenAI Chat Completions
    openai_tools = []
    ...
    # 3. Build Query-to-Context lookup map & set normalized expected_tool_calls
    ...
    try:
        ...
        print(f"\n📊 Evaluation Summary (Draft Version: {AGENT_VERSION}):")
