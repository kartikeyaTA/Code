...
object:
    ...
    one WebSocket connection. On the FIRST turn of a connection, if the client hasn't
    sent a {"type": "resume", "response_id": ...} message (see below), no
    previous_response_id is passed and the agent starts a fresh context.

    To let continuity survive a reconnect (page refresh, dropped connection) rather
    than only within one connection's lifetime, the client is expected to remember the
    response_id from the last agent_text/agent_audio message it received (e.g. in
    localStorage) and send it back as {"type": "resume", "response_id": "..."}
    immediately after receiving session_ready on the NEW connection, before starting
    to listen. The server seeds ChatSession.last_response_id from that instead of None.
...
class ChatSession:
    def __init__(self, session_id: str, ws: WebSocket) -> None:
        ...
        # previous_response_id. None until the first successful turn, unless the
        # client seeds it via a {"type": "resume", ...} message right after connecting
        # (see handle_resume / module docstring).
    ...
    def handle_resume(self, response_id: Optional[str]) -> None:
        """Seeds continuity from a PRIOR connection. Client sends this right after
        session_ready, before start_listening, carrying the last response_id it saw
        from a previous connection (e.g. persisted in localStorage across a page
        refresh). Ignored if a turn has already run on this connection — resuming
        mid-session would silently discard whatever continuity already exists here."""
        if self.last_response_id is not None:
            log.warning("[%s] ignoring resume: session already has continuity", self.session_id)
            return
        if response_id:
            self.last_response_id = response_id
            log.info("[%s] resumed conversation from response_id=%s", self.session_id, response_id)
            ...
            while True:
                ...
                if message.get("text") is not None:
                    ...
                    if msg_type == "start_listening":
                        ...
                        self.handle_resume(control.get("response_id"))
